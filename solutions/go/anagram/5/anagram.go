...
//BenchmarkDetectAnagrams-8         119636              9733 ns/op             752 B/op         65 allocs/op
...
func Detect(subject string, candidates []string) []string {
	...
	for _, candidate := range candidates {
		...
		if theMap == comparison {
			//if CompareMaps(theMap, comparison) {
...
func MakeHistogram(subject string) string {
	length := len(subject)
	hsMap := make([]byte, 26)
	for i := 0; i < length; i++ {
		char := subject[i]
...
// func CompareMaps(map1, map2 histMap) bool {
// 	for key1, val1 := range map1 {
// 		if val2, ok := map2[key1]; ok {
// 			if val1 != val2 {
// 				return false
// 			}
// 		} else {
