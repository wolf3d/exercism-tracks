...
func (v vigenere) Encode(input string) string {
	return code(input, v, CalculateRotation)
}

func (v vigenere) Decode(input string) string {
	return code(input, v, CalculateDeRotation)
}

func code(input string, v vigenere, fn func(char int, rot int, upperLimit int, basePosition int) int) string {
	rotated := strings.Builder{}
	length := len(input)
	kLength := len(v.key)
	var k int = 0
