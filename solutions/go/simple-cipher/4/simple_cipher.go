...
func (c shift) Encode(input string) string {
	return sCode(input, c, CalculateRotation)
}

func (c shift) Decode(input string) string {
	return sCode(input, c, CalculateDeRotation)
}

func sCode(input string, c shift, fn func(char int, rot int, upperLimit int, basePosition int) int) string {
	...
	for i := 0; i < length; i++ {
		if input[i] >= basePosition && input[i] <= upperLimit || input[i] >= basePositionCaps && input[i] <= upperLimitCaps {
			...
			rotated.WriteByte(byte(fn(src, c.distance, end, start)))
		}
	}
...
func (v vigenere) Encode(input string) string {
	return vCode(input, v, CalculateRotation)
}

func (v vigenere) Decode(input string) string {
	return vCode(input, v, CalculateDeRotation)
}

func vCode(input string, v vigenere, fn func(char int, rot int, upperLimit int, basePosition int) int) string {
	rotated := strings.Builder{}
	length := len(input)
	kLength := len(v.key)
	var k int = 0
	for i := 0; i < length; i++ {
		if input[i] >= basePosition && input[i] <= upperLimit || input[i] >= basePositionCaps && input[i] <= upperLimitCaps {
			...
			src := magicMap[input[i]]
			rotated.WriteByte(byte(fn(src, distance, end, start)))
		}
	}
	return rotated.String()
}
